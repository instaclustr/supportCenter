...
func TestMetricsCollector_Collect(t *testing.T) {
	...
	mockedSSHAgent.
		On("ReceiveDir",
			"/tmp/InstaclustrCollection.tar", "/some/metrics/path/snapshot", mock.AnythingOfType("scp.AcceptFunc")).
		Return(nil)
...
func TestMetricsCollector_CollectOnCompressionDisabled(t *testing.T) {
	...
	mockedSSHAgent.
		On("ReceiveDir",
			"/var/data/snapshots/20200325T090812Z-78629a0f5f3f164f", "/some/metrics/path/snapshot", mock.AnythingOfType("scp.AcceptFunc")).
		Return(nil)
